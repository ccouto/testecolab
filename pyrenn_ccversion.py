...
def NNOut(P,net,P0=None,Y0=None):
	...
	Returns:
		Y_NN: 	Neural Network output for input P
	"""
	#print(f"debug 3: {(net['layers'][-1],int(np.size(P)/net['nn'][0]))}")
...
def train_LM(P,Y,net,k_max=100,E_stop=1e-10,dampfac=3.0,dampconst=10.0,\
	...
	ErrorHistory[k]=E
	if verbose:
		print('Iteration: ',k,'		Error: ',E,'	scale factor: ',dampfac,flush=True)
	...
	#run loop until either k_max or E_stop is reached
		...
		if k>=k_max:
			...
			print('Termination Error reached',flush=True)
			break
